...
const DefaultManifestURL = "https://raw.githubusercontent.com/BarbaraNogueiraCS/runner/main/release.json"
...
func FetchManifest(ctx context.Context, manifestURL string) (Manifest, error) {
	usingDefault := strings.TrimSpace(manifestURL) == ""
	if usingDefault {
		manifestURL = DefaultManifestURL
	}

	m, err := fetchManifestRemote(ctx, manifestURL)
	if err == nil {
		return m, nil
	}

	// Durante o desenvolvimento local, o release.json novo pode ainda não estar
	// publicado na branch main do GitHub. Nesse caso, se o usuário não informou
	// uma URL manualmente, usamos o release.json local encontrado a partir do
	// diretório atual. Isso evita o erro HTTP 404 ao testar o simulador antes do
	// merge para main, mas mantém a URL remota como padrão em releases.
	if usingDefault {
		if localManifest, localErr := FetchLocalManifest(); localErr == nil {
...
func fetchManifestRemote(ctx context.Context, manifestURL string) (Manifest, error) {
...
func FetchLocalManifest() (Manifest, error) {
	path, err := FindLocalManifest()
	if err != nil {
		return Manifest{}, err
	}
	f, err := os.Open(path)
...
func FindLocalManifest() (string, error) {
	start, err := os.Getwd()
	if err != nil {
		return "", err
	}
	for {
		candidate := filepath.Join(start, "release.json")
	...
	return "", errors.New("release.json local não encontrado")
}
